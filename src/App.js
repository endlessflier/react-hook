...
const EditCustomerDialog = ({ customerInfo, onSave, onExit }) => {
  ...
  const handleSubmit = (event) => {
    ...
    const updatedErrorsState = Object.keys(defaultErrorState).reduce((result, fieldName) => {
      if (fieldName !== 'is_form_valid') {
        const error = formErrors[fieldName].required && !customerFormData[fieldName].trim().length ? true : false;
        if (error) {
          is_form_valid = false;
        }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <button
          type='button'
          onClick={handleClose}
          style={{ position: "absolute", top: "10px", right: "22px", fontSize: "22px", cursor: "pointer", border: "0", backgroundColor: "transparent" }}>
          x
        </button>
        <form onSubmit={handleSubmit} noValidate>
          <div style={{ marginBottom: "10px" }}>
            ...
            <input
              style={{ maxWidth: "100%", width: "100%", padding: "10px", boxSizing: "border-box", border: "1px solid #e1e1e1" }}
              name='first_name'
              type="text"
            ...
            {
              formErrors.first_name.error ? <ErrorValidationLabel txtLbl={formErrors.first_name.requiredText} /> : ''
            }
          </div>
          <div style={{ marginBottom: "10px" }}>
            ...
            {
              formErrors.last_name.error ? <ErrorValidationLabel txtLbl={formErrors.last_name.requiredText} /> : ''
            }
          </div>
          <div style={{ marginBottom: "10px" }}>
            ...
            {
              formErrors.address.error ? <ErrorValidationLabel txtLbl={formErrors.address.requiredText} /> : ''
            }
          </div>
          <div style={{ marginBottom: "10px" }}>
            <label style={{ display: "block", marginBottom: "5px" }}>Address 2</label>
            <textarea
              style={{ maxWidth: "100%", width: "100%", padding: "10px", boxSizing: "border-box", border: "1px solid #e1e1e1" }}
              name='address2'
              type="text"
          ...
          <div style={{ marginBottom: "10px" }}>
            ...
            {
              formErrors.city.error ? <ErrorValidationLabel txtLbl={formErrors.city.requiredText} /> : ''
            }
          </div>
          <div style={{ marginBottom: "10px" }}>
            ...
            {
              formErrors.state.error ? <ErrorValidationLabel txtLbl={formErrors.state.requiredText} /> : ''
            }
          </div>
          <div style={{ marginBottom: "10px" }}>
            ...
            <input
              ...
              required
              value={
                customerFormData.zip_code
              } />
            {
              formErrors.zip_code.error ? <ErrorValidationLabel txtLbl={formErrors.zip_code.requiredText} /> : ''
            }
          </div>
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
            <button type='submit' style={{ padding: "10px 25px", border: "0", backgroundColor: "#e1e1e1", cursor: "pointer" }}>Save</button>
          </div>
        </form>
      </div>
    </div>
...
const CustomerDirectory = () => {
  ...
  const handleFormSave = (updatedData) => {
    const customerIndex = customers.findIndex(customerObj => customerObj.customer_id === selectedCustomer.customer_id);
    if (customerIndex > -1) {
  ...
  const handleDelete = useCallback((customerData) => {
    // eslint-disable-next-line no-restricted-globals
    if (confirm(`Are you sure you want to delete this user with name ${customerData.first_name} ${customerData.last_name}`)) {
      const customerIndex = customers.findIndex(customerObj => customerObj.customer_id === customerData.customer_id);
      if (customerIndex > -1) {
        const updatedCustomersData = customers.slice()
  ...
  useEffect(() => {
    ...
    try {
      const storedCustomersData = localStorage.getItem(constants.customers_data);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap' }}>
        {customers.map(customer => <Customer key={customer.customer_id} data={customer} onEdit={handleEdit} onDelete={handleDelete} />)}
      </div>
      {
        showEditCustomerDialog ? <EditCustomerDialog customerInfo={selectedCustomer} onSave={handleFormSave} onExit={handleDialogClose} /> : ''
      }
    </div>
  );
};
